 ...
 */
export function validateStyle(style: JawStyle): ValidationResult {
  ...
  for (const [key, value] of Object.entries(style)) {
    ...
    if (UNSUPPORTED_PROPERTIES.has(key)) {
      warnings.push(
        `Property "${key}" is not supported in Jaw Beta 1 and will be ignored.`
      );
      continue;
    }
    ...
    switch (key) {
      ...
      case 'overflowX':
      case 'overflowY':
        if (!['visible', 'hidden', 'scroll'].includes(value as string)) {
