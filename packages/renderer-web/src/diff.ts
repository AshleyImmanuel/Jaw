...
export function diffTrees(
  oldTree: LayoutBox | null,
  newTree: LayoutBox | null,
): Patch[] {
  const patches: Patch[] = [];
 ...
 */
function diffNode(
  oldBox: LayoutBox | null,
  newBox: LayoutBox | null,
  path: number[],
  patches: Patch[],
): void {
 ...
 */
function hasNodeChanged(oldBox: LayoutBox, newBox: LayoutBox): boolean {
  ...
  if (oldNode.type === 'Text') {
    if (oldNode.props.content !== newNode.props.content) return true;
  }

  // Check style reference equality (quick check)
  if (oldNode.props.style !== newNode.props.style) return true;
