...
import { distributeFlexSpace, computeCrossSizes, computeMainPositions, computeCrossPositions, } from './flex';
...
function layoutNode(node, availableWidth, availableHeight) {
    ...
    if (ownWidth === undefined)
        ownWidth = availableWidth;
    if (ownHeight === undefined)
        ownHeight = availableHeight;
    // Apply constraints
    ...
    const flexChildren = childNodes.map((child, index) => {
        const childDefaults = getDefaultStyles(child.type);
        const childStyle = mergeStyles(childDefaults, child.props.style);
        ...
        const flex = childStyle.flex ?? childStyle.flexGrow ?? 0;
        // Resolve fixed sizes
        let fixedMainSize;
        let fixedCrossSize;
        if (isRow) {
            ...
            if (fixedMainSize !== undefined)
                fixedMainSize = fixedMainSize;
            if (fixedCrossSize !== undefined)
                fixedCrossSize = fixedCrossSize;
        }
        else {
    ...
    // Compute positions
    const mainPositions = computeMainPositions(flexChildren, availableMainSize, gap, justifyContent);
    const crossPositions = computeCrossPositions(flexChildren, availableCrossSize, alignItems);
    // Recursively layout each child
    const childLayoutBoxes = [];
    ...
    return createLayoutBox(node, ownWidth - boxModel.margin.left - boxModel.margin.right, ownHeight - boxModel.margin.top - boxModel.margin.bottom, boxModel, childLayoutBoxes);
}
/**
 ...
 */
function createLayoutBox(node, width, height, boxModel, children) {
    return {
        node,
        x: 0,
        y: 0,
