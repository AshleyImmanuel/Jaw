 ...
 * - undefined: all zeros
 */
export function resolveEdges(value, topOverride, rightOverride, bottomOverride, leftOverride) {
    let top = 0;
    let right = 0;
    let bottom = 0;
    let left = 0;
...
export function normalizeStyle(style) {
    ...
    if (result.margin !== undefined || result.marginTop !== undefined ||
        ...
        const edges = resolveEdges(result.margin, result.marginTop, result.marginRight, result.marginBottom, result.marginLeft);
    ...
    if (result.padding !== undefined || result.paddingTop !== undefined ||
        ...
        const edges = resolveEdges(result.padding, result.paddingTop, result.paddingRight, result.paddingBottom, result.paddingLeft);
