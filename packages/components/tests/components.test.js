...
describe('Spacer', () => {
    it('defaults to flex: 1', () => {
        ...
        expect(node.props.style.flex).toBe(1);
    });
    it('uses fixed size when provided', () => {
        const node = Spacer({ size: 20 });
        expect(node.props.style.width).toBe(20);
...
describe('Scroll', () => {
    ...
    it('sets overflow based on direction', () => {
        ...
        expect(node.props.style.overflowX).toBe('scroll');
        expect(node.props.style.overflowY).toBe('hidden');
    });
});
