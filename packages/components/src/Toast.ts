...
import { createElement, createState, createEffect } from '@jaw/runtime';
...
let activeToasts: ToastConfig[] = [];
let setToasts: ((toasts: ToastConfig[]) => void) | null = null;
 ...
 */
export function toast(message: string, type: 'success' | 'error' | 'info' = 'info', duration = 3000): void {
  if (!setToasts) {
  ...
  setTimeout(() => {
    ...
    if (setToasts) setToasts(activeToasts);
  }, duration);
}
 ...
 */
export function ToastProvider(): JawNode {
  const [toasts, setToastsSignal] = createState<ToastConfig[]>([]);
  ...
  if (currentToasts.length === 0) return Box({ style: { width: 0, height: 0 } });
